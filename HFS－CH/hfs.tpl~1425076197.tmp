...
如果只是更改选项的话,请勿修改此模板。
正确途径: 在虚拟文件系统里右键点击根目录&rarr;属性&rarr;自定义模板，
...
option.comment=1
option.rename=1
COMMENT 这些选项可以控制模板各种功能开关，请注意这些不是用户权限开关，而是全局变量!
...
[list panel]
{.if not| %number% |{:
	<div id='nothing'>{.!{.if|{.length|{.?search.}.}|无结果|无文件.}.}</div> 
:}|{:
...
<script>
	$(function(){
        if ($('#menu-panel').css('position').indexOf('sticky') < 0) // 不支持固定位置
            setInterval(function(){ $('#wrapper').css('margin-top', $('#menu-panel').height()+5) }, 300); // 固定面板请留空
    });
    ...
    <div id="additional-panels">
		...
		<div id="selection-panel" class="additional-panel" style="display:none">
			...
			<span class="buttons">
				...
				<button id="delete-selection" class="pure-button"><i class="fa fa-trash"></i><span>删除</span></button>
				<button id="move-selection" class="pure-button"><i class="fa fa-truck"></i><span>移动</span></button>
			</span>
		</div>
    </div>
</div>
...
<div id="upload-panel" class="additional-panel closeable" style="display:none">
	<div id="upload-counters">
		已上传: <span id="upload-ok">0</span> - 失败: <span id="upload-ko">0</span> - 队列: <span id="upload-q">0</span>
	</div>
	<div id="upload-results"></div>
	<div id="upload-progress">
		正在上传... <span id="progress-text"></span>
		<progress max="1"></progress>
	</div>
	<button class="pure-button" onclick="reload()"><i class="fa fa-refresh"></i> 重新加载页面</button>
</div>
...
Sorry, you lack permissions for this action=您无权执行此操作
Login=登录
Enable multi-selection=启用多选
Selection=选择
New folder=新建目录
Display timestamps=显示时间戳
Download these files as a single archive?=是否确定将文件打包下载？
Archive=打包下载
Upload=上传
User=用户
...
.item-menu-dialog .buttons { margin-top:1em }
...
[+file=folder]
        <button class='item-menu pure-button' title="更多选项"><i class="fa fa-bars"></i></button>
[+file=folder=link]
 	</div>
...
{.if| {.length|%user%.} |{:
	{.set item|{.force ansi|%folder%%item-name%.}|comment={.force ansi|上传用户: %user%.}.}
:}.}
...
{.add to log|{.force ansi|用户 %user% 创建目录.} "{.^x.}".}
{.pipe|ok.}
...
{.add to log|{.force ansi|{.if|%user%|用户 %user%|匿名用户.} 重命名.} "{.^x.}" {.force ansi|为.} "{.^yn.}".}
{.pipe|ok.}
...
{.set|log|{.force ansi|{.!Moving items to.}.} {.^dst.}.}
{.for each|fn|{.replace|:|{.no pipe||.}|{.force ansi|{.postvar|files.}.}.}|{:
    ...
    {.if not |{.exists|{.^x.}.}|{.^x.}: {.!not found.}|{:
        {.if|{.exists|{.^y.}.}|{.^y.}: {.!already exists.}|{:
            {.set|comment| {.get item|{.^x.}|comment.} .}
            {.set item|{.^x.}|comment=.} {.comment| 移动前需完成此操作，否则将会操作失败.}
            {.if|{.length|{.move|{.^x.}|{.^y.}.}.} |{:
                {.move|{.^x.}.md5|{.^y.}.md5.}
...
[special:import]
{.new account|can change password|enabled=1|is group=1|notes={.force ansi|此用户组的用户可在客户端自行修改密码。如果想允许用户更改密码，请在用户账户选择用户组中选择can change password.}.}
...
function showMsg(content, cb) {
    ...
    var ret = dialog($('<div>').css({ display:'inline-block', textAlign:'left' }).html(content), cb).css('text-align', 'center')
		.append(
		    $('<button class="pure-button">').text("{.!Ok.}").css({ margin:'1em auto 0', display:'block' })
				.click(function(ev){ ret.close() })
		)
	return ret
}//showMsg
...
function ask(msg, options, cb) {
    ...
    ret.find(':input').focus().select() // 自动聚焦功能似乎只在第一次执行时生效 :(
            ...
            try {
                ...
                data.forEach(function(r) {
                    ...
                    $(r.err ? '<span title="'+r.err+'"><i class="fa fa-ban"></i> '+ r.name+'</span>' 
						: '<a title="大小: '+r.size+'&#013;速度: '+r.speed+'B/s" href="'+r.url+'"><i class="fa fa-'+(r.err ? 'ban' : 'check-circle')+'"></i> '+r.name+'</a>')
